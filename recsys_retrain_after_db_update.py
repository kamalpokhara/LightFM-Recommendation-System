...
model = LightFM(
    no_components = 64, # old 64
    loss = "warp",
    learning_rate = 0.1, # old 0.05
    item_alpha = 1e-4,
    user_alpha = 1e-4,
    random_state = 42,
)
...
popular_df = pd.read_csv("popular_products.csv")
...
sample_users = train_events["user_id"].drop_duplicates().sample(3).tolist()
