...
print(" data loading ")
# lowecase all columns and strip whitespace
...
# Handle NaNs before they become features
...
users["registration_date"] = pd.to_datetime(users["registration_date"],format='ISO8601')
...
for interests_list in users["interests_parsed"]:
    all_user_interests.update(interests_list)
...
registered_interests = {feat for feat in user_feature_map.keys() if feat.startswith("interest:")}
def get_user_features(row):
    ...
    for interest in row["interests_parsed"]:
        interest_feat = f"interest:{interest}" 
        if interest_feat in registered_interests:
...
#parameter tuning, best so far parameter used
model = LightFM(
    no_components = 16,
    loss = "warp",
    learning_rate = 0.01,
...
for epoch in range(1, EPOCHS +1):
    ...
    if test_auc > best_test_auc :
        ...
        best_epoch = epoch
        no_improve = 0
        joblib.dump(model, "models/retrain_lightfm.pkl")
...
model = joblib.load("models/retrain_lightfm.pkl")
#10 evaluate

print(f"\n Final metrics " )
print(f"best epoch : {best_epoch}")
print(f"Best train AUC : {train_aucs[best_epoch-1]:.4f}")
...
popular_df = pd.read_csv("retrained_popular_products.csv")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("reports/lightfm_learning_curve.png", dpi=150)
print("\nPlot saved: reports/lightfm_learning_curve.png")

#12 SAVE
joblib.dump(dataset, "models/retrained_lightfm_dataset.pkl")
meta = {
...
with open("models/retrained_lightfm_meta.json", "w") as f:
    json.dump(meta, f, indent=2)

print("\nSaved:")
print("  models/retrained_lightfm_best.pkl")
print("  models/retrained_lightfm_dataset.pkl")
print("  models/retrained_lightfm_meta.json")